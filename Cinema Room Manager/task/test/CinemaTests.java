...
public class CinemaTests extends StageTest<String> {
    ...
    @DynamicTestingMethod
    CheckResult test5_checkStatistics() {
    ...
    @DynamicTestingMethod
    CheckResult test6_checkErrorHandling() {
        ...
        for (int i = 1; i < rows + 1; i++) {
            ...
            if (i == row) {
                ...
                if (splittedLine.length != columns + 1) {
                    errorMessage += "should be printed like in examples. Expected 1 number and " + columns + " symbols!";
                    throw new WrongAnswer(errorMessage);
                }
